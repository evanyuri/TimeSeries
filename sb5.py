...
WINDOW_SIZE = 30 # how many data points will we take into account to make our prediction
...
def create_uncompiled_model():
  ...
  model = tf.keras.models.Sequential([ 
    tf.keras.layers.Dense(num_features, activation='relu', name=f'layer_1'),  
    tf.keras.layers.Dense(num_features, activation='relu', name=f'layer_2'),  
...
predicted = pd.DataFrame(columns=plot_cols, data=forecast)
ints = df.index.max()
print(ints)
print(len(forecast))
future_indicies = np.arange(ints, ints + len(forecast), 1, dtype=int)
